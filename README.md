...
| Cipher symbols | `src/world/Glyphs.js` | Hex fragments and cryptographic marks adrift among the structures, drawn into a procedural canvas atlas so no font file has to resolve. They thicken around the hashing and security passages and fade with altitude, so they never silhouette against open sky in the wide shots. |
